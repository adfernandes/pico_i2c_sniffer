...
The sniffer was modernized with the lessons learned from its sibling projects
([spi_sniffer_pio](https://github.com/jjsch-dev/spi_sniffer_pio) and
[uart_sniffer_pio](https://github.com/jjsch-dev/uart_sniffer_pio)):

* **Lock-free SPSC RAM FIFO.** The inter-core buffer now uses the same
  Single-Producer Single-Consumer lock-free implementation proven in the SPI
  sniffer (volatile head/tail indices, size+1 scheme), which fixes the sync
  issue observed with the original counter-based FIFO.
* **INT (interrupt) channel.** A fifth state machine on the second PIO block
  watches an open-drain, active-low interrupt line (e.g. the INT output of a
  touch controller) and interleaves timestamped edge events with the I2C
  frames. It is configured as an input with pull-up, so when the pin is left
  unconnected it stays quiet and the output is byte-identical to the 2021
  version. It can be compiled out with `-DINT_CAPTURE_EN=OFF`.
* **Bus Pirate 5 support.** Initializes the 74HC595 cascade (VREG 3.3V rail,
  10k pull-up network, analog MUX disabled), keeps all IO buffers in input
  mode (passive by construction) and deselects the on-board SPI slaves.
* **RP2040 / RP2350 / BP5 multi-target build** with per-target PIO pin maps
  generated by CMake from `i2c_sniffer.pio.in`.

### Building

```bash
make pico1   # Raspberry Pi Pico (RP2040)   -> bin/i2c_sniffer_pio_rp2040.uf2
make pico2   # Raspberry Pi Pico 2 (RP2350) -> bin/i2c_sniffer_pio_rp2350.uf2
make bp5     # Bus Pirate 5 (RP2040)        -> bin/i2c_sniffer_pio_bp5.uf2
```

The historical `bin/i2c_sniffer_pio.uf2` (2021 build) is kept untouched for
backward compatibility.

### Pinout per target

| Signal | Pico / Pico 2 | Bus Pirate 5 |
| ------ | ------------- | ------------ |
| SDA    | GPIO0         | IO7 (BUFIO7) |
| SCL    | GPIO3         | IO0 (BUFIO0) |
| INT    | GPIO4         | IO1 (BUFIO1) |
| EV0/EV1 (internal) | GPIO1/GPIO2 | GPIO16/GPIO17 (SPI_CDI/RGB_CDO) |

On BP5, EV0/EV1 are the sniffer's internal event pins and cannot sit on the
buffered IOs (the 74LVC1T45 outputs would fight the PIO drivers), so they use
the SPI_CDI and RGB_CDO lines, which are high-impedance sinks. The on-board
NAND flash is held deselected at boot for this reason.
...
If the RAM FIFO ever overflows (capture loss), the console prints a one-time
warning with the peak FIFO level, and on the Pico the LED stays off:

```
[WARN] RAM FIFO overflow - capture loss (peak level 40123)
```